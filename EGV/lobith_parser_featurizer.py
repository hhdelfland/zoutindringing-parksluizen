...
def parse_lobith(path):
    ...
    for file in os.listdir(path):
        ...
        db['datetime'] = pd.to_datetime(
            raw_db['WAARNEMINGDATUM'] + ' ' + raw_db['WAARNEMINGTIJD (MET/CET)'], format='%d-%m-%Y %H:%M:%S')
        db = db.set_index('datetime', drop=False)
        db = db[~db.index.duplicated(keep='last')]
        db = db[np.logical_and(db['NUMERIEKEWAARDE'] <
                               1000000, db['NUMERIEKEWAARDE'] > 0)]
        lobith_dbs[file] = db
...
def featurize_lobith(lobith_db):
    ...
    rolling_window_sizes = (
        TIMESTEP_IN_HOUR, TIMESTEP_IN_HOUR*12, TIMESTEP_IN_HOUR*24)
